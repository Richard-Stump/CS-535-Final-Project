...
{
	meshes.push_back({ "bullet", new Mesh("bullet.obj") });
	meshes.push_back({ "enemy", new Mesh("enemy.obj") });
	meshes.push_back({ "powerup", new Mesh("powerup.obj") });

	//colliderMesh.load("lab3r/collider.obj");
	levelMesh.load("level.obj");
	triMesh.load("tri.obj");
}
