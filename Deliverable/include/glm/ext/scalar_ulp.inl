...
#		endif
	...
	{
		GLM_STATIC_ASSERT(std::is_floating_point<T>::value, "'next_float' only accept floating-point input");
		assert(ULPs >= 0);
...
#		endif
	...
	{
		GLM_STATIC_ASSERT(std::is_floating_point<T>::value, "'prev_float' only accept floating-point input");
		assert(ULPs >= 0);
