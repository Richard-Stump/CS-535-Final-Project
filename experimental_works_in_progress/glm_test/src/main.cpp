...
void calculate_cube(texbuffer_t *t_texbuff, glmTexturedCube* cube)
{
    // lod_t* tmp_lod = cube->get_lod();
...
void draw_cube(glmTexturedCube* cube)
{
    Mat4 mm = cube->get_model();
    Vec4 pos = cube->get_position();
    float model[16];
    for(int i = 0; i < 4; i++)
    {
        for(int j = 0; j < 4; j++)
    ...
    float position[4] = {pos.x, pos.y, pos.z, pos.w};
	create_local_world(model, position, cube->get_rotation());
...
void render(framebuffer_t *t_frame, zbuffer_t *t_z, glmTexturedCube* cube)
{
	int i, j;
	...
	for (;;)
	{
        float rs1 = 0.008f, rs2 = 6.28f, rs3 = 0.012f, rs4 = -6.28f;
        Vec3 dir1 = Vec3(1,0,0), dir2 = Vec3(1,0,0), dir3 = Vec3(0,1,0), dir4 = Vec3(0,1,0);
        Vec3 tmp_r = rs1*dir1;
		// // Spin the cube a bit.
		while (cube->get_rotation().x > 3.14f)
		{
			tmp_r += rs2*dir2;
		}
		tmp_r += rs3*dir3;
		while (cube->get_rotation().y > 3.14f)
		{
			tmp_r += rs4*dir4;
		}
		cube->add_rotation(tmp_r);
		...
		// 	for (j = 0; j < 8; j++)
		// 	{
        //         Vec4 tmp_p = Vec4(i * 40.0F, j * 40.0F, 0, 1);
		// 		cube->set_position(tmp_p);
...
int main(int argc, char *argv[])
{
    ...
    glmTexturedCube cube;

	// Init the GS, framebuffer, zbuffer, and texture buffer.
	init_gs(&frame, &z, cube.get_texbuffer_ptr());

	// Init the drawing environment and framebuffer.
	init_drawing_environment(&frame, &z);

	// Load the texture into vram.
	send_texture(cube.get_texbuffer_ptr());
