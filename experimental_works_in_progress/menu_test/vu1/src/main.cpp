 ...
 * b) Cube data (prim,lod,vertices,sts,...) added from purdue_logo_packet. 
 */
...
void calculate_cube(texbuffer_t *t_texbuff, TexturedCube* cube)
{
	packet2_add_float(cube->get_packet(), 2048.0F);					  // scale
	packet2_add_float(cube->get_packet(), 2048.0F);					  // scale
	packet2_add_float(cube->get_packet(), ((float)0xFFFFFF) / 32.0F); // scale
	packet2_add_s32(cube->get_packet(), faces_count);				  // vertex count
	packet2_utils_gif_add_set(cube->get_packet(), 1);
	packet2_utils_gs_add_lod(cube->get_packet(), cube->get_lod_ptr());
	packet2_utils_gs_add_texbuff_clut(cube->get_packet(), t_texbuff, cube->get_clut_ptr());
	packet2_utils_gs_add_prim_giftag(cube->get_packet(), cube->get_prim_ptr(), faces_count, DRAW_STQ2_REGLIST, 3, 0);
	u8 j = 0; // RGBA
	for (j = 0; j < 4; j++)
		packet2_add_u32(cube->get_packet(), 128);
}

// /** Calculate cube position and add packet with cube data */
void draw_cube(TexturedCube* cube)
{
	create_local_world(local_world, cube->get_position(), cube->get_rotation());
	...
	// Merge packets
	packet2_utils_vu_add_unpack_data(curr_vif_packet, vif_added_bytes, cube->get_packet()->base, packet2_get_qw_count(cube->get_packet()), 1);
	vif_added_bytes += packet2_get_qw_count(cube->get_packet());
...
void init_gs(framebuffer_t *t_frame, zbuffer_t *t_z, TexturedCube* cube)
{
	...
	cube->get_texbuffer_ptr()->width = 128;
	cube->get_texbuffer_ptr()->psm = GS_PSM_24;
	cube->get_texbuffer_ptr()->address = graph_vram_allocate(128, 128, GS_PSM_24, GRAPH_ALIGN_BLOCK);
...
void send_texture(TexturedCube* cube)
{
	packet2_t *packet2 = packet2_create(50, P2_TYPE_NORMAL, P2_MODE_CHAIN, 0);
	packet2_update(packet2, draw_texture_transfer(packet2->next, purdue_logo, 128, 128, GS_PSM_24, cube->get_texbuffer_ptr()->address, cube->get_texbuffer_ptr()->width));
...
void render(framebuffer_t *t_frame, zbuffer_t *t_z, TexturedCube* cube)
{
	int i, j;

	// set_lod_clut_prim_tex_buff(t_texbuffer);
	...
	calculate_cube(cube->get_texbuffer_ptr(), cube);

	// The main loop...
	for (;;)
	{
		// Spin the cube a bit.
		cube->add_rotation(0,0.008f);
		while (cube->get_rotation()[0] > 3.14f)
		{
			cube->add_rotation(0,6.28f);
		}
		cube->add_rotation(1,0.012f);
		while (cube->get_rotation()[1] > 3.14f)
		{
			cube->add_rotation(1,-6.28f);
		}
		...
		for (i = 0; i < 8; i++)
		{
			cube->set_position(0, i * 40.0F);
			for (j = 0; j < 8; j++)
			{
				cube->set_position(1, j * 40.0F);
				// cube->draw(local_world, world_view, view_screen, local_screen, camera_position, camera_rotation, vif_packets, curr_vif_packet, context, c_verts, c_sts);
                draw_cube(cube);
			}
		}
