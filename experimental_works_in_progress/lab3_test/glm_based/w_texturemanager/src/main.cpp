...
// {
// 	u32 packet_size =
// 		packet2_utils_get_packet_size_for_program(&start, &end) + 1; // + 1 for end tag
...
// void calculate_cube(texbuffer_t *t_texbuff, CubeMesh& cube)
// {
// 	packet2_add_float(cube.packet, 2048.0F);					  // scale
// 	packet2_add_float(cube.packet, 2048.0F);					  // scale
// 	packet2_add_float(cube.packet, ((float)0xFFFFFF) / 32.0F); // scale
// 	packet2_add_s32(cube.packet, faces_count);				  // vertex count
	
	
// 	packet2_utils_gif_add_set(cube.packet, 1);
	
	
// 	packet2_utils_gs_add_lod(cube.packet, &lod);
	
	
// 	packet2_utils_gs_add_texbuff_clut(cube.packet, t_texbuff, &clut);
	
	
// 	packet2_utils_gs_add_prim_giftag(cube.packet, &prim, faces_count, DRAW_STQ2_REGLIST, 3, 0);
	
	
	
// 	u8 j = 0; // RGBA
// 	for (j = 0; j < 4; j++)
// 		packet2_add_u32(cube.packet, 128);
// }
...
// void render(glmCamera* cam, texbuffer_t *t_texbuff, CubeMesh& cube)
// {
// 	int i, j;
...
//     glmCamera cam(Vec3(0,2,20));
// 	CubeMesh cube(Vec3(0,1,0));
// 	PS2Pad gamepad;
...
VECTOR object_rotation = {100, 140.00f, 300, 1.0f};
...
// Mat4 rotation_matrix_z(float val)
// {
//     Mat4 rot = Mat4(1.0);
//     rot[0][0] = cos(val);
//     rot[0][1] = sin(val);
...
// Mat4 create_local_world(Vec3 translation, Vec3 rotation)
// {
//     Mat4 ret = Mat4(1.0);
//     ret = glm::rotate(ret, rotation);
...
void calculate_cube(texbuffer_t *t_texbuff)
{
	...
	for (j = 0; j < 4; j++)
		...
		// packet2_add_u32(zbyszek_packet, 0);
		// packet2_add_u32(zbyszek_packet, 0);
		// packet2_add_u32(zbyszek_packet, 128);
}

/** Calculate cube position and add packet with cube data */
void draw_cube(glmCamera& cam, VECTOR t_object_position, texbuffer_t *t_texbuff)
{
	create_local_world(local_world, t_object_position, object_rotation);
	// local_world = create_local_world(t_object_position, object_rotation);
    create_world_view(world_view, camera_position, camera_rotation);
	create_view_screen(view_screen, graph_aspect_ratio(), -3.00f, 3.00f, -3.00f, 3.00f, 1.00f, 2000.00f);
	create_local_screen(local_screen, local_world, world_view, view_screen);
	...
	// Add matrix at the beggining of VU mem (skip TOP)
	packet2_utils_vu_add_unpack_data(cam.curr_vif_packet, 0, &local_screen, 8, 0);
	...
	// Merge packets
	packet2_utils_vu_add_unpack_data(cam.curr_vif_packet, vif_added_bytes, zbyszek_packet->base, packet2_get_qw_count(zbyszek_packet), 1);
	vif_added_bytes += packet2_get_qw_count(zbyszek_packet);

	// Add vertices
	packet2_utils_vu_add_unpack_data(cam.curr_vif_packet, vif_added_bytes, c_verts, faces_count, 1);
	vif_added_bytes += faces_count; // one VECTOR is size of qword

	// Add sts
	packet2_utils_vu_add_unpack_data(cam.curr_vif_packet, vif_added_bytes, c_sts, faces_count, 1);
	vif_added_bytes += faces_count;
...
void send_texture(texbuffer_t *texbuffer)
{
    packet2_t *packet2 = packet2_create(50, P2_TYPE_NORMAL, P2_MODE_CHAIN, 0);
	packet2_update(packet2, draw_texture_transfer(packet2->next, bedrich, 128, 128, GS_PSM_24, texbuffer->address, texbuffer->width));
	packet2_update(packet2, draw_texture_flush(packet2->next));
	dma_channel_send_packet2(packet2, DMA_CHANNEL_GIF, 1);
	dma_wait_fast();
	packet2_free(packet2);
...
void clear_screen(glmCamera& cam)
{
	packet2_t *clear = packet2_create(35, P2_TYPE_NORMAL, P2_MODE_NORMAL, 0);

	// Clear framebuffer but don't update zbuffer.
	packet2_update(clear, draw_disable_tests(clear->next, 0, &cam.z));
	packet2_update(clear, draw_clear(clear->next, 0, 2048.0f - 320.0f, 2048.0f - 256.0f, cam.frame.width,  cam.frame.height, 0x40, 0x40, 0x40));
	packet2_update(clear, draw_enable_tests(clear->next, 0, &cam.z));
...
void render(glmCamera& cam, texbuffer_t *t_texbuff)
{
	int i, j;
	...
	for (;;)
	{
		...
		// {
			...
			// {
				c_zbyszek_position[1] = 160.0F;
				draw_cube(cam, c_zbyszek_position, t_texbuff);
			// }
		// }
